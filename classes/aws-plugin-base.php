...
class AWS_Plugin_Base {
	...
	function __construct( $plugin_file_path ) {
		...
		$this->aws_sdk_version	= defined( '\Aws\Sdk::VERSION' ) ? \Aws\Sdk::VERSION : null;
	}

	/**
	 * Accessor for AWS SDK version
	 *
	 * @return mixed
	 */
	public function get_aws_sdk_version() {
		return $this->aws_sdk_version;
	}
	...
	function get_plugin_page_url() {
		return network_admin_url( 'options-general.php?page=' . $this->plugin_slug );
	}
	...
	function plugin_row_meta_links( $links, $file ) {
		...
		$row_meta = array(
			'original_author'    => 'Original version by <a href="' . esc_url( 'https://deliciousbrains.com/' ) . '" target="_blank" aria-label="' . esc_attr__( 'Original Author', 'amazon-web-services' ) . '">Delicious Brains</a>'
		);
