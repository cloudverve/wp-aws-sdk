<div class="aws-content aws-settings">
	...
	<?php elseif ( ! $use_ec2_iam_roles && ( $this->are_prefixed_key_constants_set() || $this->are_key_constants_set() ) ) : ?>
		...
		<?php if ( ! $this->are_access_keys_set() ) : ?>
			<div class="notice-error notice">
				<p>
					<?php _e( 'Please check your <tt>wp-config.php</tt> file as it looks like one of your defines is missing or incorrect.', 'amazon-web-services' ); ?>
				</p>
			</div>
		<?php endif; ?>

		<p>
			<?php printf( __( 'You&#8217;ve already defined your AWS access keys in your <tt>wp-config.php</tt>. If you&#8217;d prefer to manage them here and store them in the database (not recommended), simply remove the lines.', 'amazon-web-services' ), array(
				'utm_campaign' => 'support+docs',
			) ); ?>
		</p>
	...
	<?php else : ?>
		...
		<p>
			<?php _e( 'We recommend defining your Access Keys in <tt>wp-config.php</tt> so long as you don&#8217;t commit it to source control (you shouldn&#8217;t be). Simply copy the following snippet and replace the stars with the keys.', 'amazon-web-services' ); ?>
		</p>
